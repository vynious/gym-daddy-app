    ...
    <a-row :gutter="16">
      <a-col :span="8" v-for="classItem in classes" :key="classItem.id">
        ...
        >
          ...
          <b>Difficulty: {{ classItem.suitable_level.toUpperCase() }}</b>
          <br />
  ...
  methods: {
    ...
    fetchBookedClasses() {
      ...
      axios
        .get(bookingURL, {
          headers: {
            Authorisation: `Bearer ${JSON.parse(
        ...
        .then((response) => {
          ...
          this.bookedClasses = response.data.bookings.map(
            (booking) => booking.class_id
          );
        })
        .catch((error) => {
          console.error(
            "There was an error fetching the booked classes data:",
            error
          );
        });
    },
